...
import com.baidu.shop.service.PayService;
	...
	*/
    ...
    public void requestPay(PayInfoDTO payInfoDTO,String token, HttpServletResponse response) {
        ...
        try {
            ...
            Result<OrderInfo> info = orderFeign.getOrderInfoByOrderId(Long.valueOf(payInfoDTO.getOrderId()));
            if(info.getCode() == 200){

                OrderInfo data = info.getData();

                List<String> title = data.getOrderDetailEntityList().stream().map(orderDetali -> orderDetali.getTitle()).collect(Collectors.toList());
                String titleStr = String.join(",", title);
                ...
                //付款金额，必填
                String total_amount = Double.valueOf(data.getActualPay()) / 100 + "";
                //订单名称，必填
